/**
 * RACE: a repack committing MID-CLONE — the delta-pack design's central safety
 ...
 * `clone` mode: a full clone of a NEVER-repacked repo, repack fired mid-flight,
 * so the racing pass adds the repo's FIRST encodings while the serve is reading.
 *
...
const ITERS = 12
const RUNS = 600
const TEMPLATE = "race/clone-repack/template/full-raw"
 ...
 * of the serve — unlike a rewind race, a repack that starts after the clone
 * finished exercises nothing. */
const DELAY_FRACTIONS = [
	0, 0.03, 0.08, 0.15, 0.22, 0.3, 0.4, 0.5, 0.6, 0.72, 0.85, 0.97,
] as const

describe("race — repack committing mid-clone (never-repacked repo)", () => {
	let db: IsolatedDb
	...
	beforeAll(async () => {
		...
		await store.putPack(TEMPLATE, fullObjects)
		for (const { name, oid } of fullRefs) {
			await refs.setRef(TEMPLATE, name, oid)
		}
		await refs.setSymref(TEMPLATE, "HEAD", head)

		// The anchored identity proof (docs/2026-08-20-test-efficiency.md): one copy,
		// checked against the same canonical object/ref identity the seeding
		// established. EMPTY encodings, not unchecked: "never repacked, so every
		// object serves raw" is the whole premise, and it is what the copy must carry.
		...
		const calDest = join(mkdtempSync(join(tmpdir(), "race-cr-clone-cal-")), "c")
	...
	it("a clone raced by a repack lands complete and fsck-clean", async () => {
		...
		for (let i = 0; i < ITERS && breaks.length === 0; i++) {
			...
			const repo = `race/clone-repack/clone/${i}`
			const dest = join(mkdtempSync(join(tmpdir(), "race-cr-clone-")), "c")
			scratch.push(dest)
