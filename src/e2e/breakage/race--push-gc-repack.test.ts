...
describe("race — a wire push, a gc(), a repack() and a clone all in flight", () => {
	...
	it("an acked push stays servable, a clone that says OK is sound, and the repo converges", async () => {
		const breaks: string[] = []
		...
		for (let i = 0; i < ITERS && breaks.length === 0; i++) {
			...
			// Q1 — a push git reported as SUCCESSFUL must stay fetchable.
			if (pushErr === undefined) {
				...
				if (serverTip !== pushed) {
					...
					try {
						...
						problems.push(`ACKED PUSH NOT SERVABLE: ${msg(e)}`)
					}
			...
			try {
				...
				// A repo whose ref is intact but which no longer clones is permanent
				// damage; if the push was rejected AND the ref never moved, the same
				// symptom is just the orphan race and is reported separately.
				problems.push(`DID NOT CONVERGE: ${msg(e)}`)
			}
