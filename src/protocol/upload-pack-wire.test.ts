 ...
 *
 * These executable goldens define the required wire output. They codify M0/M1
 * wire correctness and lock it against regression.
 *
...
describe("§8.1 upload-pack wire — surface 1: v2 capability advertisement", () => {
	...
	// honored cap set: `fetch=filter include-tag thin-pack` (thin-pack landed
	// with spine S5), not git's `fetch=shallow wait-for-done` (§4.2.3 —
	// advertise only what we honor). Template over agent + algo.
