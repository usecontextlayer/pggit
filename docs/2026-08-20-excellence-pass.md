...
### Cycle C — cycleCore:performance-testing-code — passes done, close pending
...
Structural hypothesis (to be confirmed by profiling): the race family's cost is dominated by per-iteration re-seeding of the same immutable multi-thousand-object set into fresh repos via `putPack` (clone-vs-repack: 30 iters × 3 modes, up to 2 seeds per round of a ~10k-object set; err-pkt-overflow: 20 × 10k; concurrent-repack: 40 × 3.5k; short-pack: 60 × 2k; two-pushes: 25 × 3.5k) — fixture assembly wrapped around race windows of a few seconds.

Candidate content fixes (owner sign-off required before landing): (a) template-repo seeding — seed once per suite, per-iteration DB-side row copy into a fresh repo_id (identical never-repacked state, ~100ms vs seconds; new fixture pattern); (b) delta-only second seeds in half/fetch modes (full set minus base set — matches what a real push carries; currently the second putPack re-sends every base object to be conflict-skipped); (c) ITERS trims that preserve each file's full delay-sweep exactly once (e.g. clone-vs-repack 30→12 covers all 12 delays × 3 modes once).

In-flight: instrumented solo profiling run of `race--clone-vs-repack` (temporary UNSTAGED scratch: env-gated git-subprocess timing in `src/testing/spawn-git.ts`, query counting in `src/testing/pg.ts`, untracked `vitest.profile.config.ts` adding `--cpu-prof` to the fork pool). Output decomposition: git wall per subcommand, PG round-trips, JS CPU flamegraph, residual. The scratch is reverted after the mission; it must never be committed.

### Remaining after cycle C close

1. **File-rename + folder re-org pass** (owner-ordered, "at some point", before or early in cycle D): execute the verified reorg plan with recorded rulings — `lang.ts` extraction YES; `repo-view/` → `repo-file` naming + snapshots→projection collapse executed as ONE `refactor!` commit; `repack.ts` exports `createRepack`; full `.spec` suffix strip; mechanics: `codegraph sync` first, `git mv` + per-file import edits (never sed), commit-per-coherent-move. The full 26KB plan text lives in the driving job's tmp (`reorg-plan.md`) — copy it somewhere durable or re-derive before that job is deleted.
...
- The perf Postgres on localhost:6489 is shared and long-lived — never restart it. The docker-compose Postgres is machine-global. Kill only exact PIDs you spawned.
...
**Open questions, and what they gate.** (1) Template-repo DB-copy: sanctioned as a fixture pattern? — gates efficiency fix (a). (2) Is "full delay-sweep exactly once" acceptable race coverage? — gates fix (c). (3) Are the efficiency reductions part of cycle C's close or a separate commit track? — gates commit sequencing. (4) When may the next full gate run? — gates cycle-C close and everything after it.
