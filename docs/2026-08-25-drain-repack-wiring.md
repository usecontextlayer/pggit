...
The drain (`src/store/gc-scheduler.ts`) runs GC only; repack (`src/store/repack.ts`, the producer of the derived pack-encoding tier) is host-invoked only, so nothing in pggit's own server ever brings a repo's encoding tier to coverage. Delta-pack D5 decided the shape long ago: per repo the drain runs GC then repack over survivors, with `repos.last_repack_at` (migration 0008) as repack's own watermark — stamped by `repack()` itself on success since D15, because the repair trigger depends on the stamp marking a *completed* pass. W1(a) is purely the scheduler layer plus config plumbing: no migration, no store change, no wire change.
...
**No observability change.** `drainOnce()`'s summary remains the only reporting surface; the platform-side log line is the ContextLayer wiring's decision (gc-unwired note, question 2). The delta doc's "promote the instrument counters" idea stays parked with Task B.
...
**Docs updated in the same commit:** `repack.ts`'s module doc Scheduling note (pggit's own drain is now an invoker; standalone callers still stamp via `repack()` itself), `gc-scheduler.ts`'s module doc (two-phase pass), `env.ts` comments, a short dated pointer addendum in `docs/2026-06-24-gc-scheduler-design.md` (§5/§6 extended by this doc), and the delta doc's W1(a) entry annotated as landed.
...
- **`repack: { wholes: 0, deltas: 0 }` and `repack: null` are different claims.** Zeros = the phase ran and found nothing pending (real case: a ref-delete push advances `last_pushed_at` with no new objects; repack runs, stamps via its early return, reports zeros). Null = the phase did not run. Tests must assert the distinction, never treat them as interchangeable.
...
- `index.ts` already exports `DrainEntry`/`DrainSummary` and `RepackResult` (verified 2026-08-25) — the reshape flows through existing exports; nothing new to export.
...
- The same-commit doc updates are listed in the verification section above; nothing beyond them surfaced.
