...
4. THEN: the worker-count A/B on an owner-scheduled full gate (ruling 7) — the race family no longer anchors the critical path.
...
6. Correction pending owner review (session audit, 2026-08-21): the decomposition figures labelled "PG round-trips" are postgres.js query-execution counts from the driver's `debug` hook, taken over a `max: 4` pool — so the "× RTT ⇒ ≥23 s serialized floor" arithmetic is a model ceiling, not a measurement. The counts themselves stand; the round-trip framing and the floor claim await the owner's read of the audit before re-wording.
7. PARKED, gated on an owner ruling: rolling template-copy across the rest of the race family (the old sequencing line). The audit found the anchored identity proof narrower than its claim — `assertCanonicalStoreFixture` asserts oid sets in 5 of the 9 tables `copyTemplateRepo` carries and no non-oid column (not `git_commit.generation`, the column the copy exists to carry verbatim) — and the suites already converted are the ones where the uncovered tables are empty. Strengthen the proof before any further rollout; the trims have shrunk the family's remaining seeding share regardless.
