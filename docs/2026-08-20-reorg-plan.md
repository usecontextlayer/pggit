...
## EXECUTION LEDGER (driver, 2026-08-21 → 2026-08-24 session; written mid-flight at step 15)

Steps 1–14 + the `createRepack` export are COMMITTED locally (UNPUSHED — origin still at `119dcac`): step 1 `5b37e30`, step 2 `7df788c` (lens-foldered probes + `@perf/*` alias + repo-wide alias sweep of `migrate.ts`/`manage.ts` + AGENTS.md convention line), step 3 `74c2590` (README), step 4 `8bf2f9c` (regressions lens folders, prefix drop, solo globs repointed, describe titles → `regressions/<lens> — …`), step 5 `2a58fd1`, step 6 `23d6c1d`, step 7 `b026624`, step 8 `1c923e6`, step 9 `214c5b2`, step 10 `190ddc2`, step 11 `c04a008`, step 12 `8cc0928`, steps 13+14 `1cf2272` (**`refactor(repo-file)!:` — the gitc generator omitted the `!`; amended via `git commit --amend -F` keeping the generated body, which already carried the `BREAKING CHANGE:` footer**), createRepack export `4d14c81` (`feat`). Statics (`tsc -b --force` after deleting `dist-types/tsconfig.tsbuildinfo`, `biome check --error-on-warnings`) green at every boundary; `tsdown` additionally green at steps 2, 13+14, and the export. `vitest list` after step 4: 654 tests discovered, solo project = exactly the intended 8 files. Full `git diff --no-ext-diff` review (owner-directed) done for steps 1–4; steps 5–14 verified by exact-string edits + residue greps + tsc.

**Step 15 is IN THE TREE, UNCOMMITTED**: all 28 `.spec.test.ts → .test.ts` renames staged (zero collisions), ~12 comment cross-references updated (incl. `refname.spec`/`gc-scheduler.spec` shorthand mentions), the findings-ledger disposition-table line updated with a dated note, `pkt-oracle.ts`/`repo-commands.test.ts` prose reworded where the infix distinction died. The plan's expected vitest-config comment rewrite is MOOT — the current config never mentions `.spec` (the vitest-native gate rewrite already dropped it); the `**/gc-scheduler*.test.ts` solo glob still matches the renamed `generative/gc-scheduler.test.ts`. Remaining for step 15: tsc + biome, `vitest list` (expect 654 / solo 8 incl. that file), `git diff --no-ext-diff` review, `gitc`.

**Execution-time deviations from the plan text, all recorded above as rulings**: `memory.ts`+`memory.test.ts` stay at `perf/` root (census: 5 probes import it — the plan's privacy claim was stale); the pg-txn death test's runtime child now emits at the REPO ROOT — cycle C's tsconfig `include` widening to `perf/**` had SILENTLY stripped the "outside include, stray copy can't break tsc" protection the old `perf/breakage/` location relied on, and root-emit restores it (`_MAPPING-pg-txn.md` note updated); Kanel-generated `src/database/models/**` is exempt from the alias mandate (generator emits relative imports; hand-edits would be overwritten at the next `manage.ts codegen` — AGENTS.md records the exception; if the owner wants generated files aliased too, the fix is kanel config, not hand edits); frozen tombstone cells (`lifecycle--repack-bind-parameter-ceiling`, `pg-bloat--git-edge-quadratic`, `_txn-util`, `_wire-util`) deliberately NOT repointed — their targets no longer exist.

**After step 15 commits, still owed**: one dated top note each on the archival docs (`2026-06-23-reorg-design.md`, `2026-06-24-gc-scheduler-design.md`, `2026-06-26-read-surface-sharpening-design.md`, and the 2026-08-19 findings ledger for its frozen appendices) mapping the renames, as one docs commit; a dated subjective layer in the excellence doc; `git push` (1Password); whatever test RUN the owner schedules (none run this session — discovery only; the no-full-suite rule held). **Then PAUSE — the owner directed: no cycle D applychecks without explicit permission.**

## DRIVER RULING + EXECUTION — e2e prefix foldering (owner, 2026-08-24)

Owner, after the 15-step pass landed: "i think we can do a little more re-org in the e2e tests folder. mostly anything with the same prefix in the filename can be a folder" → "do it!!" — **superseding REJECTED #9's flat-and-greppable call for the behaviour tree** (the 2026-08-21 lens-folder logic, extended to flat `src/e2e/`). Executed same day: the six prefix clusters became folders with prefixes dropped — `push/` (15), `fetch/` (8), `gc-scheduler/` (5), `transport/` (5), `gc/` (4), `pack-encoding/` (4) — 41 pure `git mv`s; the 14 non-cluster files stay flat (9 singletons + the 5 serve/frontier spine files `bitmap-serve`/`thin-pack-serve`/`served-set-oracle`/`frontier-provenance`/`reach-epoch-producer`, whose would-be `serve/` folder is a CONCEPT grouping, not a prefix — offered to the owner as a separate ruling, not taken unilaterally). `gc-` split by family: `gc-scheduler-*` → `gc-scheduler/` (the must-run-alone family), never `gc/scheduler-*`. CONSEQUENCE, same class as step 4's: the solo glob `**/gc-scheduler*.test.ts` matched basenames — replaced in the same commit by `**/gc-scheduler/*.test.ts` (the e2e family) + `**/gc-scheduler.test.ts` (the generative differential); `vitest list` after: 654 tests, solo = the same 8 files. Zero import edits (nothing imports an e2e file); 9 comment cross-references repointed across 7 files; docs per the standing policy — dated-entry docs keep their names under EXTENDED top notes (spine, efficiency, excellence, findings ledger, 2026-06-23), and `2026-08-15-delta-pack-design.md`, found citing lens-renamed suites with NO note since the 2026-08-21 reorg, got one covering both waves. The owner scheduled the FULL battery (first-ever full run over the reorganized tree) immediately after this commit; it ran green same day — **169/169 files, 654/654 tests, 682 s wall at load ~4–5**.
