...
- **Per-repo advisory lock + multi-instance claim** (`FOR UPDATE SKIP LOCKED` / `pg_advisory_xact_lock(repos.id)`) — deferred until pggit runs >1 instance; v1 is single-instance with the in-process overlap guard (`inFlight`). The data model already extends to it (claim on the eligible `repos` row). **⚠ Prerequisite HALVED 2026-08-16 (delta-pack D12):** the staging-table half of the old hard prerequisite is GONE — the live set is now a TEMP table on the pass's own reserved connection, session-private and collision-proof across instances by construction (the shared `gc_live_<id>` UNLOGGED table this paragraph originally warned about no longer exists). The advisory-lock half STANDS: before a second instance runs, the lock is still wanted for write-path interlock and to avoid duplicated sweep work (see gc.ts's concurrency comment).
