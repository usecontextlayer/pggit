...
| Scheduler permanently settles young garbage | **RECORDED** — real liveness gap (denied-push residue younger than grace never requalifies); needs a `next_gc_at`/watermark design on the scheduler. |
| Direct GC self-deadlocks at `max: 1` (maintain borrows the pool) | **FIXED** — `maintain()` runs on the pass's reserved connection. |
| Delta decoding accepts truncated instructions / synthesizes zeros | **RECORDED** — real permissive-ingest divergence from canonical `index-pack`; touching the oracle-tested delta decoder deserved its own gated change, not a session-tail edit. |
...
| Invalid refnames persist and poison framing | **RECORDED** — needs `check-ref-format` semantics incl. D/F conflicts at the receive boundary. |
| New branches may point at non-commit objects | **RECORDED** — conformance gap vs canonical receive-pack; needs a type check before CAS. |
...
| Connectivity verifies existence, not required type | **RECORDED** — typed-edge validation (commit tree→tree, parent→commit, mode→type) at ingest. |
...
| fetch-multiround enshrines the buggy no-pack-after-ready behavior; canonical sibling skipped | **RECORDED, flagged for the user** — changing the multiround response shape is a deliberate parked decision (`fetch-ready-sibling.skip.test.ts` documents it); flipping wire behavior unsupervised at session end was judged wrong. This is the one CRITICAL deliberately left standing. |
