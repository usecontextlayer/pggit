...
- **`git_commit`** and **`git_tag`** are derived rows carrying the graph's queryable shape — a commit's root tree, ORDERED parents, committer time, and generation number; a tag's target and type — written transactionally at ingest and backfilled by migration. Reachability walks read these rows plus tree content directly (batched primary-key point-reads), so fetch negotiation and the push connectivity check never re-parse commit bodies and never depend on a statistics-sensitive recursive join.
